  ...
  <form method="POST" action="{{route('payments.update', $payment->id)}}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 mt-4 gap-4">
            ...
            <div class="flex flex-col">
                ...
                <select required id="students" class="p-2" name="user_id" value="{{old('user_id', $payment->user_id)}}">
                    @foreach ($students as $student)
                        <option 
                         value={{$student->id}}
                         @if ($student->id == $payment->user_id) selected @endif
                         >
                         {{$student->name}}
                        </option>
                    @endforeach
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 mt-4 gap-4">
            ...
            <div class="flex flex-col">
                ...
                <input type="checkbox" name='paid' @if ($payment->paid) checked @endif/>
            </div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 mt-4 gap-4">
            <div class="flex flex-col">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 mt-4 gap-4">
            <button type="submit" class="bg-green-700 p-2 text-white w-full mb-2 max-w-lg">Editar</button>
        </div>
    </form>
</section>
@endsection
